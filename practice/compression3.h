...
//uses a hash chain for pattern matching
...
namespace qfs {
	...
	class HashChain {
		private:
			...
			Match getMatch(uint prevPos, uint pos) {
				...
				if((length >= 3 && offset <= 1024) || (offset <= 16384 && length >= 4) || length >= 5) {
		...
		public:
			...
			Match getLongestMatch(uint pos) {
				...
				while(prevPos != 0xFFFFFFFF && n_chains < 100) {
					Match match = getMatch(prevPos, pos);
	...
	bytes compress(bytes& src) {
		//finding patterns
		HashChain chain = HashChain(src);
		vector<Match> matches = vector<Match>();
		
		uint i = 0;
		while(i <= src.size() - 3) {
			chain.addTo(i);
			Match match = chain.getLongestMatch(i);
