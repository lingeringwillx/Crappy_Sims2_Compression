...
namespace dbpf {
	...
	bytes compressEntry(Entry& entry, bytes& content, int level) {
		if(!entry.compressed && !entry.repeated) {
			...
			int length = qfs_compress(&content[0], content.size(), &newContent[0], level);
	...
	bytes decompressEntry(Entry& entry, bytes& content) {
		if(entry.compressed) {
			...
			bool success = qfs_decompress(&content[0], content.size(), &newContent[0], newContent.size(), false);
