...
func parallelCompress(entries []Entry, ch chan []Entry, recompress bool, decompress bool) {
	for i := 0; i < len(entries); i++ {
...
func main() {
	...
	flag.BoolVar(&parallel, "parallel", false, "")
	flag.BoolVar(&recompress, "r", false, "Recompress")
	flag.BoolVar(&recompress, "recompress", false, "")
	...
	if(parallel && len(pack.entries) > 1) {
		nCPU := runtime.NumCPU()
		
		if nCPU > len(pack.entries) {
			nCPU = len(pack.entries)
		}
		
		ch := make(chan []Entry, nCPU)
		...
		partitionLength := totalLength / nCPU
		i := 0
		j := 0
		
		//begin parallel compression
		for nCores := 0; nCores < nCPU - 1; nCores++ {
			length := 0
			...
			go parallelCompress(pack.entries[i:j], ch, recompress, decompress)
			
			i = j
		}
		
		//put the all remaining entries here
		go parallelCompress(pack.entries[i:], ch, recompress, decompress)
	...
	error = os.WriteFile(fileName + ".new", packPackage(&pack), 'w')
	
	if error != nil {
		fmt.Println("Could not write new file")
		return
	}
	
	error = os.Chmod(fileName + ".new", 0600)
	
	if error != nil {
		fmt.Println("Could not make file writable")
		os.Remove(fileName + ".new")
		return
	}
	
	error = os.Rename(fileName + ".new", fileName);
	
	if error != nil {
		fmt.Println("Could not overwrite old file")
		os.Remove(fileName + ".new")
		fmt.Println(error)
		return
	}
